#!/usr/bin/env python3
# GRABMAP v5 - DoomXR feature-showcase ninja course, built from the swarm's validated spec.
...
# FIRING/PARRY spur, RELOAD RANGE (manual-reload gauntlet), SPIN PORTAL (rotating polyobject
# portal, folded in from XR_SpinPortal.pk3). v5 also self-contains its own ZSCRIPT/SCRIPTS lumps
# (XRTestMenu + XRSDFShowcase + GravCoaster's XR_GravPlank + XR_PolyPortalCarrier) so `-file
# GRABMAP.wad` alone is fully playable -- no companion pk3s required any more.
# Generated geometry, validated (closure/dangling/portal/things/gutters).
import struct, sys, json, os, zipfile
...
# =============== SPIN PORTAL (rotating polyobject portal, folded in from XR_SpinPortal.pk3) ===============
# Simplified single-doorway version of the original 4-way spinning-cube demo: one square
# polyobject spins in place forever; its west edge is a live Line_SetPortal seam linked to a
# static alcove a short walk away. XR_PolyPortalCarrier (embedded ZSCRIPT below) is what actually
# carries the player through as the moving seam sweeps past -- native portal code only does that
# for an actor crossing a STATIONARY line, not a line moving under a stationary actor.
POLYID = 1
platform(720,-380,1080,-20,0,ceilh=256,ftex="FLOOR4_8",bot="METAL",name="spinbay")
alc=SEC(0,256,"FLOOR0_1","CEIL1_1",170)
al_bl=V(1150,-260); al_br=V(1300,-260); al_tr=V(1300,-140); al_tl=V(1150,-140)
LN(al_bl,al_br, SD(room, mid="METAL"), back=SD(alc), block=False)
LN(al_br,al_tr, SD(room, mid="METAL"), back=SD(alc), block=False)
LN(al_tr,al_tl, SD(room), back=SD(alc), block=False, special=156, args=(950,0,3,0,0), lid=951)
LN(al_tl,al_bl, SD(room, mid="METAL"), back=SD(alc), block=False)
footprints.append(("spinalcove",1150,-260,1300,-140))
poly=SEC(0,256,"FLOOR4_8","F_SKY1",190)
pcx,pcy,phalf=900,-200,100
p_bl=V(pcx-phalf,pcy-phalf); p_br=V(pcx+phalf,pcy-phalf); p_tr=V(pcx+phalf,pcy+phalf); p_tl=V(pcx-phalf,pcy+phalf)
LN(p_bl,p_br, SD(poly, mid="METAL"), back=-1, block=True, special=1, args=(POLYID,0,0,0,0))   # Polyobj_StartLine
LN(p_br,p_tr, SD(poly, mid="METAL"), back=-1, block=True)
LN(p_tr,p_tl, SD(poly, mid="METAL"), back=-1, block=True)
# portal edge needs a real closed backing sector (can't just point back at `room` -- that leaves
# room's own boundary loop with two stray unconnected vertices and fails closure). `prcv` is a
# tiny inert dummy sector, coincident with the polyobject, that exists solely to close the loop.
prcv=SEC(0,256,"FLOOR0_1","CEIL1_1",150)
pr_a=V(pcx-phalf-40,pcy-phalf); pr_b=V(pcx-phalf-40,pcy+phalf)
LN(p_bl,pr_a, SD(prcv), back=-1, block=True)
LN(pr_a,pr_b, SD(prcv), back=-1, block=True)
LN(pr_b,p_tl, SD(prcv), back=-1, block=True)
LN(p_tl,p_bl, SD(poly), back=SD(prcv), block=False, special=156, args=(951,0,3,0,0), lid=950)  # rotating portal edge
THING(pcx,pcy,9300,POLYID)   # polyobject anchor at the pivot (angle field = poly id)
# (spinpoly deliberately NOT added to footprints -- it's meant to sit inside spinbay, not beside it)
SDF(288,pcx,pcy-260,220,"1.00,1.00,1.00","pulse,spin-2")
SDF(0,1225,-200,180,"0.10,0.90,1.00","pulse")
...
def _read_pk3_lump(pk3name, lump):
    here=os.path.dirname(os.path.abspath(__file__))
    with zipfile.ZipFile(os.path.join(here,pk3name)) as z:
        return z.read(lump).decode()

def build_zscript():
    here=os.path.dirname(os.path.abspath(__file__))
    core_menu=open(os.path.join(here,"..","..","wadsrc","static","zscript","ui","vr_test_menu.zs")).read()
    sdf=_read_pk3_lump("XRSDFShowcase.pk3","xr_sdf_showcase.zs")
    grav=_read_pk3_lump("GRAVCOASTER.pk3","xr_gravcoaster.zs")
    poly=_read_pk3_lump("XR_SpinPortal.pk3","zscript/xr/xr_polyportalcarrier.zs")
    parts=[
      "version \"4.5\"\n",
      "// ==== folded in from wadsrc/static/zscript/ui/vr_test_menu.zs (orphaned core copy, never wired into zscript.txt) ====\n",
      core_menu,
      "\n// ==== folded in from XRSDFShowcase.pk3 ====\n",
      sdf,
      "\n// ==== folded in from GRAVCOASTER.pk3 ====\n",
      grav,
      "\n// ==== folded in from XR_SpinPortal.pk3 (FishyClockwork's Polyobject Line Portal Assistant, GPL-3.0) ====\n",
      poly,
    ]
    return "".join(parts).encode("utf-8")

def build_scripts():
    return (b'#include "zcommon.acs"\n\n'
            b'script "XRSpinPortal" OPEN\n'
            b'{\n'
            b'    while (1)\n'
            b'    {\n'
            b'        Polyobj_RotateLeft(%d, 8, 360);\n'
            b'        PolyWait(%d);\n'
            b'    }\n'
            b'}\n' % (POLYID,POLYID))
...
if not err:
    zmap=(b'DoomEdNums { 30500 = XR_GravPlank }\n'
          ...
          b'    EventHandlers = "XR_PolyPortalCarrierHandler"\n}\n')
    # v5 folds XRTestMenu + XRSDFShowcase + GravCoaster's XR_GravPlank + XR_SpinPortal's carrier
    # straight into this wad's own ZSCRIPT/SCRIPTS lumps -- GRABMAP.wad alone is now fully
    # self-contained, no companion pk3s need to be loaded alongside it.
    # NOTE: SCRIPTS is shipped as plain ACS SOURCE (no compiled BEHAVIOR) -- GZDoom's built-in ACS
    # compiler compiles it at map load. Untested here (no engine to launch); if the spin portal
    # doesn't rotate, check the console for an ACS compile error first.
    blob=wad([("ZMAPINFO",zmap),("ZSCRIPT",build_zscript()),("GRABMAP",b""),("TEXTMAP",emit()),
              ("SCRIPTS",build_scripts()),("ENDMAP",b"")])
