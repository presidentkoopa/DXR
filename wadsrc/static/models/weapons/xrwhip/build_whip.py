...
# [XR] NOTE: the SHIPPED whip skin bound by modeldef is XRWhipLeather.png, generated by the sibling
# make_leather_skin.py (organic worn-leather grain). write_whip_skin() below produces a diamond-PLAIT
# lattice which read in-headset as a WICKER BASKET, so its output (xrwhip_skin.png) is NOT the bound
# skin. If you re-point the modeldef, use make_leather_skin.py's output, not this one.
