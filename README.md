...
VR_UpdateWeaponAnim — new native function, peer to VR_UpdateArmIK, runs every tic in P_PlayerThink. On each shot it kicks the held gun's hs_grip root bone back-and-up, then springs it to rest — fluid recoil, no baked frames, no re-export. It writes the same proceduralPose channel the whip and marine body already use, which the HUD weapon render consumes (confirmed the path end-to-end: RenderHUDModel → RenderFrameModels(psp->Caller) → ProcessModelFrame, and every weapon modeldef already has modelsareattachments).
