...
void HWFlat::DrawFlat(HWDrawInfo *di, FRenderState &state, bool translucent)
{
	...
	if (hasGlow)
	{
		...
		auto& tn = sector->ceilingplane.Normal();
		auto& bn = sector->floorplane.Normal();
		FVector4 tp = { (float)tn.X, (float)tn.Y, (float)sector->ceilingplane.negiC, (float)sector->ceilingplane.fD() };
		FVector4 bp = { (float)bn.X, (float)bn.Y, (float)sector->floorplane.negiC, (float)sector->floorplane.fD() };
	...
	bool wallGlowOn = false;
	{
		...
		int wn = 0;
		if (sector->GetWallGlowHeight() > 0 && wn < MAX_WALL_GLOW_SPOTS)
		{
			PalEntry wc = sector->GetWallGlowColor();
	...
	if (wallGlowOn) state.EnableWallGlow(false);
