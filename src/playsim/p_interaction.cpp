...
void P_TouchSpecialThing (AActor *special, AActor *toucher)
{
	...
	{
		// Wallet logic
		if (special->vr_metadata.value > 0)
		{
			toucher->player->Wallet += special->vr_metadata.value;
		}

		// Haptic feedback
		if (special->vr_metadata.haptic_type.IsNotEmpty())
		{
			VR_HapticEvent(special->vr_metadata.haptic_type.GetChars(), 0, 100 * special->vr_metadata.haptic_intensity, 0, 0);
		}
	}
...
static int DoDamageMobj(AActor *target, AActor *inflictor, AActor *source, int damage, FName mod, int flags, DAngle angle)
{
	if (vr_allow_weapon_parrying && target && target->player && inflictor && (inflictor->flags & MF_MISSILE))
	{
		...
		if (VR_CheckWeaponParry(target->player, inflictor, &hand))
		{
			...
			if (vr_parry_spark)
			{
				for (int i = 0; i < 24; i++)
				{
					DVector3 sparkVel = DVector3(M_Random.Random2(), M_Random.Random2(), M_Random.Random2()) / 32.0;
					P_SpawnParticle(target->Level, inflictor->Pos(), sparkVel, DVector3(0, 0, -0.4), PalEntry(255, 180, 220, 255), 1.0, 15, 3.0, 0.9, 0.0, 0);
				}
			}
	...
	if (vr_crit_chance > 0 && inflictor && source && source->player && ((target->flags3 & MF3_ISMONSTER) || target->player))
	{
		if ((M_Random() / 65535.0) < vr_crit_chance)
		{
			damage = int(damage * vr_crit_mult); // Critical hit multiplier
			if (vr_show_locational_text)
			{
				FVRMSDFTextThinker::SpawnText(target->Level, target->PosPlusZ(target->Height * 0.5), FName("Arcade"), "CRITICAL!", 35, 1.2, NAME_None);
			}
			
			// We can play a special sound or effect here if we want.
			S_Sound(target, CHAN_AUTO, CHANF_NONE, "misc/critical", 1.0, ATTN_NORM);
		}
	}
	...
	// VR Cumulative Damage Numbers
	if (realdamage > 0 && source && source->player && target && target != source)
	{
		FVRMSDFDamageCounter::RecordDamage(target, realdamage);
	}
