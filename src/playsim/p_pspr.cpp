	...
	for (int i = 0; i < MAXPLAYERS; i++)
	{
		if (playeringame[i])
		{
			...
			while (pspr)
			{
				...
				// VR WEAPON SHELL HOOK: Advance 3D Tics
				if (vr_weapon_shell && pspr->Caller != nullptr && pspr->Caller->IsKindOf(NAME_Weapon))
				{
					if (pspr->ID == PSP_WEAPON || pspr->ID == PSP_OFFHANDWEAPON)
					{
						AActor* weapon = pspr->Caller;
						if (weapon->vr_weapon_data && weapon->vr_weapon_data->Current3DState)
						{
...
void DPSprite::SetState(FState *newstate, bool pending)
{
	...
	// VR WEAPON SHELL HOOK: Animation Synchronization (State Mapping)
	if (vr_weapon_shell && Caller != nullptr && Caller->IsKindOf(NAME_Weapon))
	{
		AActor* weaponActor = Caller;
		AActor* weapon = weaponActor;
		if (weapon->vr_weapon_data && weapon->vr_weapon_data->Archetype != EVRWeaponArchetype::Unknown)
		{
			if (ID == PSP_WEAPON || ID == PSP_OFFHANDWEAPON)
			{
				...
				if (archClass && State != nullptr)
				{
					...
					if (!archState)
					{
						archState = archClass->FindState(NAME_Ready);
					}
