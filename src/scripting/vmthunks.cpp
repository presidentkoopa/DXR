...
DEFINE_ACTION_FUNCTION_NATIVE(_Sector, SetXOffset, SetXOffset)
{
 ...
 static void SetGlowSpot(sector_t *self, int color, double radius, double x, double y)
 {
	 self->SetGlowSpot(color, float(radius), x, y);
 }

 DEFINE_ACTION_FUNCTION_NATIVE(_Sector, SetGlowSpot, SetGlowSpot)
 {
	 PARAM_SELF_STRUCT_PROLOGUE(sector_t);
	 PARAM_COLOR(color);
	 PARAM_FLOAT(radius);
 ...
 static void AddGlowSpot(FLevelLocals *self, int color, double radius, double x, double y, int planeFlags)
 {
	 if (radius > 0.0)
	 {
		 FGlowSpot gs = { DVector2(x, y), radius, PalEntry(color), 0, 0.0, DVector2(0, 0), planeFlags };
		 self->GlowSpots.Push(gs);
	 }
 }

 DEFINE_ACTION_FUNCTION_NATIVE(_LevelLocals, AddGlowSpot, AddGlowSpot)
 {
	 PARAM_SELF_STRUCT_PROLOGUE(FLevelLocals);
	 PARAM_COLOR(color);
	 PARAM_FLOAT(radius);
	 PARAM_FLOAT(x);
	 PARAM_FLOAT(y);
	 PARAM_INT(planeFlags);
		 AddGlowSpot(self, color, radius, x, y, planeFlags);
	 return 0;
 }

 static void AddGlowSpotWiped(FLevelLocals *self, int color, double radius, double x, double y, int wipeType, double wipeProgress, double dirX, double dirY, int planeFlags)
 {
	 if (radius > 0.0)
	 {
		 FGlowSpot gs = { DVector2(x, y), radius, PalEntry(color), wipeType, wipeProgress, DVector2(dirX, dirY), planeFlags };
		 self->GlowSpots.Push(gs);
	 }
 }

 DEFINE_ACTION_FUNCTION_NATIVE(_LevelLocals, AddGlowSpotWiped, AddGlowSpotWiped)
 {
	 PARAM_SELF_STRUCT_PROLOGUE(FLevelLocals);
	 PARAM_COLOR(color);
	 PARAM_FLOAT(radius);
	 ...
	 PARAM_FLOAT(y);
	 PARAM_INT(wipeType);
	 PARAM_FLOAT(wipeProgress);
	 PARAM_FLOAT(dirX);
	 PARAM_FLOAT(dirY);
	 PARAM_INT(planeFlags);
		 AddGlowSpotWiped(self, color, radius, x, y, wipeType, wipeProgress, dirX, dirY, planeFlags);
	 return 0;
 }
	...
	static void AddGlowPanel(FLevelLocals *self, int color, double radius, double x, double y, double z, int wipeType, double wipeProgress, double dirX, double dirY, int counter)
	{
		if (radius > 0.0)
		{
			FGlowSpot gs = { DVector2(x, y), radius, PalEntry(color), wipeType, wipeProgress, DVector2(dirX, dirY), 0, z, 1, counter };
			self->GlowSpots.Push(gs);
		}
	}

	DEFINE_ACTION_FUNCTION_NATIVE(_LevelLocals, AddGlowPanel, AddGlowPanel)
	{
		PARAM_SELF_STRUCT_PROLOGUE(FLevelLocals);
		PARAM_COLOR(color);
		PARAM_FLOAT(radius);
...
DEFINE_FIELD_X(Line, line_t, Keywords)
