...
bool FGLRenderState::ApplyShader()
{
	...
	activeShader->muWalletValue.Set(players[consoleplayer].Wallet);
	activeShader->muScoreValue.Set((int)players[consoleplayer].score);
	...
	if (mGlowEnabled || mWallGlowEnabled || activeShader->currentglowstate)
	{
		...
		activeShader->muWallGlowSpots.Set(&mStreamData.uWallGlowSpots[0].X, MAX_WALL_GLOW_SPOTS);
		activeShader->muWallGlowSpotCount.Set(mStreamData.uWallGlowSpotCount);
		activeShader->muWallGlowMask.Set(&mStreamData.uWallGlowMask[0].X, MAX_WALL_GLOW_SPOTS);
		activeShader->currentglowstate = mGlowEnabled || mWallGlowEnabled;
	}
