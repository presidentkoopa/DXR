...
void PPArcadeCRT::Render(PPRenderState *renderstate, double timer, int sceneWidth, int sceneHeight)
{
	if (!vr_arcade_crt)
		return;

	renderstate->PushGroup("arcade_crt");

	ArcadeCRTUniforms uniforms;
	uniforms.Timer = (float)timer;
	uniforms.Resolution = FVector2((float)sceneWidth, (float)sceneHeight);
...
void PPDataHaze::Render(PPRenderState *renderstate, double timer)
{
	if (!vr_data_haze)
		return;

	renderstate->PushGroup("data_haze");

	DataHazeUniforms uniforms;
	uniforms.Timer = (float)timer;
...
{
	double timer = I_msTime() / 1000.0;
	exposure.Render(state, sceneWidth, sceneHeight);
	dataHaze.Render(state, timer);
...
void Postprocess::Pass2(PPRenderState* state, int fixedcm, float flash, int sceneWidth, int sceneHeight)
{
	...
	arcadeCrt.Render(state, timer, sceneWidth, sceneHeight);
	lens.Render(state);
	fxaa.Render(state);
