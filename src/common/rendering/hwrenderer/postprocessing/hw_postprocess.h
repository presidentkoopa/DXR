...
struct ArcadeCRTUniforms
{
	...
	static std::vector<UniformFieldDesc> Desc()
	{
		return
		{
			{ "Timer", UniformType::Float, offsetof(ArcadeCRTUniforms, Timer) },
			{ "Padding0", UniformType::Float, offsetof(ArcadeCRTUniforms, Padding0) },
			{ "Resolution", UniformType::Vec2, offsetof(ArcadeCRTUniforms, Resolution) }
		};
	}
};
...
{
public:
	void Render(PPRenderState *renderstate, double timer, int sceneWidth, int sceneHeight);

private:
	PPShader Shader = { "shaders/pp/arcade_crt.fp", "", ArcadeCRTUniforms::Desc() };
};

struct DataHazeUniforms
{
	float Timer;
	float Density;
	FVector2 Padding0;
...
{
public:
	void Render(PPRenderState *renderstate, double timer);

private:
	PPShader Shader = { "shaders/pp/data_haze.fp", "", DataHazeUniforms::Desc() };
};
