...
bool FGLRenderState::ApplyShader()
{
	...
	if (mHwUniforms)
	{
		...
		activeShader->cur->muVisualRegime.Set(mHwUniforms->mVisualRegime);
		activeShader->cur->muArcadeCRT.Set(mHwUniforms->mArcadeCRT);
		activeShader->cur->muAffineWarp.Set(mHwUniforms->mAffineWarp);
	}
	...
	if (mGlowEnabled || mWallGlowEnabled || activeShader->cur->currentglowstate)
	{
		...
		activeShader->cur->muWallGlowSpots.Set(&mStreamData.uWallGlowSpots[0].X, MAX_WALL_GLOW_SPOTS);
		activeShader->cur->muWallGlowSpotCount.Set(mStreamData.uWallGlowSpotCount);
		activeShader->cur->muWallGlowMask.Set(&mStreamData.uWallGlowMask[0].X, MAX_WALL_GLOW_SPOTS);
		activeShader->cur->currentglowstate = mGlowEnabled || mWallGlowEnabled;
	}
