...
bool FShader::Load(const char * name, const char * vert_prog_lump_, const char * frag_prog_lump_, const char * proc_prog_lump_, const char * light_fragprog_, const char * defines)
{
	...
	i_data += R"(
		...
		uniform	int uVisualRegime;
		uniform	int uArcadeCRT;
		uniform	int uAffineWarp;
		...
		uniform highp vec4 uWallGlowSpots[MAX_WALL_GLOW_SPOTS];
		uniform int uWallGlowSpotCount;
		uniform highp vec4 uWallGlowMask[MAX_WALL_GLOW_SPOTS];
...
//
	...
	shaderData->muVisualRegime.Init(shaderData->hShader, "uVisualRegime");
	shaderData->muArcadeCRT.Init(shaderData->hShader, "uArcadeCRT");
	shaderData->muAffineWarp.Init(shaderData->hShader, "uAffineWarp");
	...
	shaderData->muWallGlowSpots.Init(shaderData->hShader, "uWallGlowSpots");
	shaderData->muWallGlowSpotCount.Init(shaderData->hShader, "uWallGlowSpotCount");
	shaderData->muWallGlowMask.Init(shaderData->hShader, "uWallGlowMask");
